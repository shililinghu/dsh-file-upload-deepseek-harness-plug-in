  ...
  factory: (require) => {
    ...
    const DICTIONARIES = {
      "zh-CN": {
        ...
        "zip.extracted": "已上传并解压 zip：`{path}` → `{dir}/`，共 {count} 个文件{skipped}{via}：{listed}{more}。请读取解压后的文件。",
        "zip.via": "（由 {tool} 解压）",
      ...
      en: {
        ...
        "zip.extracted": "Uploaded and extracted zip: `{path}` → `{dir}/`, {count} file(s){skipped}{via}: {listed}{more}. Please read the extracted files.",
