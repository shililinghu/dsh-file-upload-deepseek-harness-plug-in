    ...
    function UploadedFilesRow(props) {
      ...
      async function removeOne(item) {
        ...
        try {
          ...
          uploadStore.remove(item.path);
        } catch (err) {
          setError(err instanceof Error ? err.message : String(err));
        }
      }

      /** Delete every uploaded file on the host, then clear the list. */
        ...
        items.map((item, index) =>
          React.createElement("span", {
            ...
            React.createElement("button", {
              ...
              onMouseEnter: (event) => { event.currentTarget.style.background = "rgba(229,72,77,.25)"; event.currentTarget.style.opacity = "1"; },
              onMouseLeave: (event) => { event.currentTarget.style.background = "transparent"; event.currentTarget.style.opacity = "0.75"; },
              style: removeStyle,
            }, "✕")
          )
        ),
