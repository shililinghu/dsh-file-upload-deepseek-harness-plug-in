window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** zh / en dictionaries for this plugin's namespace. */
    const DICTIONARIES = {
      "zh-CN": {
        ...
        "zip.extracted": "已上传并解压 zip：`{path}` → `{dir}/`，共 {count} 个文件{skipped}：{listed}{more}。请读取解压后的文件。",
        "zip.skippedNote": "（跳过 {count} 个不支持条目）",
        "zip.more": "等 {count} 个文件",
      ...
      en: {
        ...
        "zip.extracted": "Uploaded and extracted zip: `{path}` → `{dir}/`, {count} file(s){skipped}: {listed}{more}. Please read the extracted files.",
        "zip.skippedNote": " ({count} unsupported entries skipped)",
        "zip.more": "and {count} more",
      ...
      function describeUpload(result) {
        if (result.kind === "zip") {
          if (result.extracted) {
            ...
            const more = files.length > MAX_LISTED_FILES ? t("zip.more", { count: files.length }) : "";
    ...
    function apply(ctx) {
      ...
      const disposeDict = ctx.locale.register(NS, DICTIONARIES);
      ctx.effect ? ctx.effect(disposeDict) : disposeDict();
      ...
      }, (props) => React.createElement(UploadButton, Object.assign({}, props, { t }))));
      ctx.slots.inject("settings.general.item", () => ctx.slots.register({
        name: "settings.general.item",
        id: "agent-hub-workspace-file-upload-limit",
        order: 30,
        label: t("slot.uploadLimitLabel"),
      }, (props) => React.createElement(UploadLimitRow, Object.assign({}, props, { t }))));
    }
