...
/** Hard caps so a hostile or broken archive cannot exhaust disk/memory. */
export const MAX_EXTRACT_BYTES = 256 * 1024 * 1024
export const MAX_EXTRACT_ENTRIES = 4096
...
export async function extractZip(buffer, destination, limits = {}) {
  ...
  for (let index = 0; index < entryCount; index += 1) {
    if (index >= maxEntries) throw new Error(`ZIP entry count exceeds the ${maxEntries} limit.`)
    ...
    if (rawName.endsWith('/') || rawName.endsWith('\\')) {
      await mkdir(join(root, ...safeName.split('/')), { recursive: true })
      continue
    }
    ...
    if (totalBytes > maxBytes) throw new Error(`Extracted size exceeds the ${maxBytes} byte limit.`)
